...
public class OreMeltedType extends MeltedType {
    ...
    protected OreMeltedType(@NotNull ElementType elementType) {
        ...
        this.name = IRDock.getPlugin().getLanguageManager().getComponent(null, "ore_melted_type_name." + elementType.name().toLowerCase());
        this.meltedName = IRDock.getPlugin().getLanguageManager().getComponent(null, "ore_melted_type_melted_name." + elementType.name().toLowerCase());
    }
